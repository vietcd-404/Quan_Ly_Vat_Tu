class Controller:
    def __init__(self, model, view):
        self.model = model
        self.view = view
        self.view.create_button.config(command=self.create_data)
        # self.view.read_button.config(command=self.read_data)
        self.view.update_button.config(command=self.update_data)
        self.view.delete_button.config(command=self.delete_data)
    ...
    def read_data_kho_hang(self):
        results = self.model.read("Loai_Vat_Tu")
        if results is not None:
            self.view.selected_item_combobox['values'] = ()
            self.view.data_mapping = {}  # Clear the existing mapping

            # Update the combobox with new values and store the mapping
            display_values = []
            for row in results:
                display_values.append(row[1])  # Display row[1] in the combobox
                self.view.data_mapping[row[1]] = row[0]  # Map row[1] to row[2]

            self.view.selected_item_combobox['values'] = display_values
            if display_values:  # Check if the display_values list is not empty
             self.view.selected_item_combobox.current(0)
        else:
            self.view.show_message("Lỗi dữ liệu")        
